...
module.exports = {
    ...
    run(message, args, client) {
        ...
        if (member.roles.cache.has(Config.Roles.Erkek)) member.roles.remove(Config.Roles.Erkek).catch(e => { });
        if (message.guild.roles.cache.get(Config.Roles.Uye)) member.roles.add(Config.Roles.Uye).catch(e => { });
        member.roles.add(Config.Roles.Kadin).catch(e => { })
        ...
        if (isim && yas) {
            ...
            mmb.findOne({ userID: member.id }, (err, data) => {
                if (!data) { new mmb({ userID: member.id, regSize: 1, cv: [`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")} **Cinsiyet:** Kadın. `] }).save() } else {
                    data.regSize++, data.cv.push(`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")}, İsim: ${isim} ${yas} **Cinsiyet:** Kadın.`), data.save()
                }
            })
            ...
            mmb.findOne({ userID: member.id }, (err, data) => {
                if (!data) { new mmb({ userID: member.id, regSize: 1, cv: [`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")} `] }).save() } else {
                    data.regSize++, data.cv.push(`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")}, İsim: ${isim} **Cinsiyet:** Kadın.`), data.save()
                }
            })
            ...
            mmb.findOne({ userID: member.id }, (err, data) => {
                if (!data) { new mmb({ userID: member.id, regSize: 1, cv: [`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")} `] }).save() } else {
                    data.regSize++, data.cv.push(`**Kayıt Eden:** <@${message.author.id}>, **Tarih:** ${moment(Date.now()).format("** HH.mm - DD.MM.YY **")}, **İsim:** Belirtilmedi,  **Cinsiyet:** Kadın.`), data.save()
                }
            })
        }
